...
export default function EditPhenomenaPage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (phenomenonRes.ok) {
        const phenomenonData = await phenomenonRes.json();
        setPhenomenon(phenomenonData);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Failed to update phenomenon');
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="text-lg text-red-600 mb-4">Phenomenon not found</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              <h1 className="ml-4 text-xl font-semibold">Edit Fenomena</h1>
            </div>
          </div>
        </div>
      </nav>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {currentUser?.role !== 'ADMIN' && (
                <p className="mt-2 text-sm text-gray-500">
                  Anda hanya dapat mengedit fenomena di wilayah yang telah ditetapkan
                </p>
              )}
            </div>
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              >
                {submitting ? 'Menyimpan...' : 'Update Fenomena'}
              </button>
            </div>
          </form>
        </div>
